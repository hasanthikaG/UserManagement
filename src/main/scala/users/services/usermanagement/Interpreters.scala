...
object Interpreters {
  def default(userRepository: UserRepository)(implicit ec: ExecutionContext): Algebra[Future[?]] =
    new DefaultInterpreter(userRepository)

  def unreliable(underlying: Algebra[Future[?]], config: ServicesConfig.UsersConfig)(
      implicit ec: ExecutionContext
  ): Algebra[Future[?]] = new UnreliableInterpreter(underlying, config)
}

final class DefaultInterpreter private[usermanagement] (userRepository: UserRepository)(implicit ec: ExecutionContext)
    extends Algebra[Future[?]] {
  import User._

  private def generateId(): Future[Id] = Future.successful(Id(UUID.randomUUID().toString))

  private def save(user: User): Future[Error Either Done] =
    for {
      result ← userRepository.insert(user)
    } yield Right(result)

  def get(id: Id): Future[Error Either User] =
    for {
      maybeUser ← userRepository.get(id)
      result = maybeUser.toRight(Error.NotFound)
    } yield result

  def signUp(username: Username, email: EmailAddress, password: Password): Future[Error Either User] =
    (for {
      ...
      result ← EitherT.fromEither[Future] {
        if (maybeUser.nonEmpty) Left(Error.Exists: Error)
          ...
          Right(User(id, username, email, password, OffsetDateTime.now()))
      }
      _ ← EitherT(save(result))
    } yield result).value

  def updateEmail(id: Id, email: EmailAddress): User =
    (for {
      user ← EitherT(get(id))
      result ← EitherT.fromEither[Future] {
        if (user.isDeleted) Left(Error.Deleted)
        else Right(user.updateEmailAddress(email, OffsetDateTime.now()))
      }
      _ ← EitherT(save(result))
    } yield result).value

  def updatePassword(id: Id, password: Password): Future[Error Either User] =
    (for {
      user ← EitherT(get(id))
      result ← EitherT.fromEither[Future] {
        if (user.isDeleted) Left(Error.Deleted)
  ...
  def resetPassword(id: Id): Future[Error Either User] =
    (for {
      user ← EitherT(get(id))
      result ← EitherT.fromEither[Future] {
        if (user.isDeleted) Left(Error.Deleted)
  ...
  def block(id: Id): Future[Error Either User] =
    (for {
      user ← EitherT(get(id))
      result ← EitherT.fromEither[Future] {
        if (user.isDeleted) Left(Error.Deleted)
  ...
  def unblock(id: Id): Future[Error Either User] =
    (for {
      user ← EitherT(get(id))
      result ← EitherT.fromEither[Future] {
        if (user.isDeleted) Left(Error.Deleted)
  ...
  def delete(id: Id): Future[Error Either Done] =
    (for {
      user ← EitherT(get(id))
      result ← EitherT.fromEither[Future] {
        if (user.isDeleted) Left(Error.Deleted)
...
final class UnreliableInterpreter private[usermanagement] (
  ...
  def signUp(
    ...
    failOrTimeout(underlying.signUp(userName, email, password))
